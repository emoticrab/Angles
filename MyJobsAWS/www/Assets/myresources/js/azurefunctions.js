...
function refreshOrLogin(callback) {
    opMessage("refreshOrLogin " + new Date());
    refreshToken(function (result) {
        if (result == "SUCCESS") {
            opMessage("refreshOrLogin refresh Token SUCCESS " + new Date());
            callback("SUCCESS")
        }
        else {
            opMessage("refreshOrLogin refresh Token FAIL try Login " + new Date());
            loginAzure(function (result) {
                opMessage("refreshOrLogin loginAzure callback " + result + " " + new Date());
                callback("SUCCESS")
            })
        }
    })
...
function loginAzure(callback) {
    mobileService.login('aad').done(function (results) {
        ...
        opMessage("loginAzure SUCCESS - userId = " + userId + " " + new Date());
        localStorage.setItem("token", mobileService.currentUser.mobileServiceAuthenticationToken)
        localStorage.setItem("user", mobileService.currentUser.userId)
        ...
        opErrorMessage("loginAzure FAIL - error = " + error + " " + new Date());
        callback(error)
    });
}
...
function logoutazure() {
    ...
    mobileService.logout(function () {
        ...
        localStorage.setItem("user", null)
    	 ...
    	 opMessage("error Logging out od Azure");
    });
}
function logoutazureandrestart() {
    ...
    mobileService.logout(function () {
        ...
        opMessage("error Logging out od Azure after DB Reset");
    });
}
...
function getAzureUsername(callback) {
    ...
    mobileService.invokeApi("/../.auth/me", {
        ...
        if (typeof results.result === "undefined") {
         ...
         //   opMessage("getAzureUsername  SUCCESS " + results.result[0].user_id + " " + new Date());
            callback(results.result[0].user_id.toUpperCase());
        }
    }, function (error) {
        opMessage("getAzureUsername FAIL " + error + " " + new Date());
        callback("FAIL")
    });
}
...
setSyncingIndicator(true) 
    $.ajax({
        ...
        opMessage("SYNC:UPLOAD Azure call success. In done function for: " + page);
...
function requestAzureData(page, params) {
    ...
    opMessage("requestAzureData " + page + " " + params + " " + new Date());
    ...
    // if (Date.parse(localStorage.getItem('LASTTOKENREFRESH')) < d || mobileService.currentUser == null) {
       if(1==1){  //refresh token every time
            opMessage("requestAzureData  LASTTOKENREFRESH= " + localStorage.getItem('LASTTOKENREFRESH') + " " + new Date());
            
            refreshOrLogin(function (result) {
                if (result == "SUCCESS") {
                    opMessage("requestAzureData refreshOrLogin SUCCESS " + new Date());
                    d.setMinutes(d.getMinutes() + 50)
                    SetConfigParam('LASTTOKENREFRESH', d);
                }
                opMessage("requestAzureData Calling getAzureData1 " + new Date());
                
                getAzureData(page, params);
            })
        }
        else {
            opMessage("requestAzureData Calling getAzureData2 " + new Date());
            getAzureData(page, params, function (result) {
                if (result.messageType == "F") {
                    opMessage("getAzureData2 returned F " + new Date());
                    refreshOrLogin(function (result) {
                        if (result == "SUCCESS") {
                            opMessage("requestAzureUsername refreshOrLogin SUCCESS " + new Date());
                            d.setMinutes(d.getMinutes() + 50)
...
function getAzureData(azurepage, params) {
  ...
  $.ajax({
       ...
       // opMessage("getAzureData Calling getAzureData SUCCESS " + new Date());
        if (azurepage == "getUserScenario"){
        ...
        opErrorMessage("getAzureData Calling getAzureData FAIL " + new Date());
...
function chooseCB(mydata, page) {
    switch (page) {
      ...
      /*  case "ZGW_GET_JOB_DETAILS":
        ...
        case "ZGW_MAM30_001_GET_JOB_DETAILS":
            getLocalJobs(mydata);
            ...
            if (type == "EquipmentStatus")// Process New Notifications  
            {
                ...
                function (transaction, results, rowsArray) {
                    if (rowsArray.length > 0) {
                        ...
                        html5sql.process("UPDATE MyNotifications SET notifno = 'SENDING' WHERE id='" + item['id'] + "'",
                        function () {
                            postAzureData("ZGW_EQSTATUS_UPDATE", myjson, "UPDATE MyNotifications SET notifno = 'NEW' WHERE id='" + item['id'] + "'", item['id']);
