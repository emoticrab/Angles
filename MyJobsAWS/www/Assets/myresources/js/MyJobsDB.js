...
function opLog(msg,type){
    ...
    console.log("XXXXXX" + sqlstatement);
		html5sql.process(sqlstatement,
						 function(){
...
//changes for CREOL added new parameter equipment_status
...
html5sql.process("INSERT INTO  MyNotifications (notifno , type, startdate, starttime, shorttext, longtext , priority , pgroup , pcode , grouptext, codetext, funcloc, equipment, assignment, SpecialReq, reportedby, reportedon, plant , orderno, funclocgis, equipmentgis, assigntome, equipmentstatus) VALUES (" +
					 "'NEW','" + type + "','" + startdate + "','" + starttime + "','" + description + "','" + details + "','" + priority + "','" + group + "','" + code + "','" + grouptext + "','" + codetext + "','" + funcloc + "','" + equipment + "','" + employeeno + "','" + SpecialReq + "','" + ReportedBy + "','" + ReportedOn + "','','','','','" + assigntomechar + "','" + equipment_status + "');",
	 function(transaction, results, rowsArray){
...
function syncTransactional(){
	 var d = new Date() 
	 //opMessage("SYNC:Called syncTransactional");
	...
	if (Date.parse(localStorage.getItem('LastLogoff')) < d) {
	    ...
	    if (!CheckSyncInterval('TRANSACTIONAL')) {
	      //  opMessage("SYNC:TRANSACTIONAL SyncInterval not reached");
	        return;
	    }
...
function syncUpload() {
    opMessage("SYNC:UPLOAD Calling syncUpload");
    

	var UploadSQLStatement=""
if(!checkConnection()){
    opMessage("SYNC:UPLOAD checkConnection false - exiting syncUpload");
		return;
	}
...
if (Date.parse(localStorage.getItem('LastLogoff')) < d) {
    d.setDate(d.getDate() + 1);
    SetConfigParam('LastLogoff', d);
    opMessage("SYNC:UPLOAD Logout due - exiting syncUpload and logging out");
    logoutazure();
    return;
}
	...
	UploadSQLStatement="select 'VehicleCheck' as type, '' as extra,id as id, recordupdated from MyVehicleCheck where state = 'NEW' "
		...
		UploadSQLStatement+=" union "
    /*changes for CREOL*/UploadSQLStatement += " select 'EquipmentStatus' as type,   shorttext as extra,id    as id, recordupdated from MyNotifications where notifno = 'NEW' and type <> 'Z7' "
        UploadSQLStatement+=" union "
...
function syncReference() {
    ...
    if (Date.parse(localStorage.getItem('LastLogoff')) < d) {
        d.setDate(d.getDate() + 1);
        SetConfigParam('LastLogoff', d);
        opMessage("SYNC:REFERENCE Logout due - exiting syncUpload and logging out");
        logoutazure();
        return;
    }
...
//added new table MyObjectList for CR work bundling
...
//added equipmentstatus field in Myjobclose table and Mynotofications table
					 ...
					 "INSERT INTO MyWorkConfig (paramname , paramvalue ) VALUES ('TRACE','OFF');"+
					 "INSERT INTO MyWorkConfig (paramname , paramvalue ) VALUES ('SYNC_REFERENCE_FREQUENCY','8400000');"+
...
sqlstatement+="INSERT INTO MyWorkConfig (paramname , paramvalue ) VALUES ('SYNC_REFERENCE_FREQUENCY','8400000');";
...
function createMyJobsDir() {
    ...
    window.resolveLocalFileSystemURL(cordova.file.externalRootDirectory + "Documents/", function (fileEntry) {
        ...
        opMessage('Cannot Resolve Device Storage - Error =' + error.code);
    });
