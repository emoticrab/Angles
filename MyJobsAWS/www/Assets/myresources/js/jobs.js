...
var demodatafordropdown=[{Code:"RIT",Description:"Insufficient time",SAPCode:"RIT-Insuff.Time",Visibility:"UC"},
							...
							new sap.m.Table("MPTable",{
								...
								columns:[ 
										new sap.m.Column({header: new sap.m.Label({text:"Id"}),
							        	 ...
							        	 hAlign: 'Left',width: '15%', minScreenWidth : "" , demandPopin: false}),//end of changes
...
html5sql.process("select distinct mp.object_id, mp.meas_point, mp.psort, mp.pttxt , mp.unit_meas, mpd.code, mpd.value, mpd.shorttext ,c.equipment ,c.equidescr from myjobdetsmpoints as mp left JOIN mympointdocs as mpd  ON mp.meas_point = mpd.meas_point  inner join MyObjectList c on mp.object_id = c.equipment where object_id = '"+objectID+"' order by mp.meas_point",
				...
				while (n < rowsArray.length) {
					...
					opTable.addItem (new sap.m.ColumnListItem({
						cells : 
							...
							new sap.m.Text({text: rowsArray[n].equidescr}),
                   ...
                   if ((sap.ui.getCore().byId("StatusRejectReasonTextMultiReject").getValue().length > 0) &&
                       ...
                       for (row = 0; row < opTable.getItems().length ; row++) {
                           if (opTable.getItems()[row].getCells()[0].mProperties.selected == true) {
                               rejectJob(opTable.getItems()[row].getCells()[4].mProperties.text,
     	                	    	       	    	            										...
     	                	    	       	    	            										columns:[
     	                	    	        	    	            										     ...
     	                	    	        	    	            										     new sap.m.Column({header: new sap.m.Label({text:"Completed"}),
     	                	    	       	    	            										         ...
     	                	    	       	    	            										         new sap.m.Column({header: new sap.m.Label({text:"Feedback Code"}),
     	                	    	       	    	            										        	...
     	                	    	       	    	            										        	new sap.m.Column({header: new sap.m.Label({text:"Completed Description"}),
     	                	    	       	    	            										        	 hAlign: 'Left',width: '25%',minScreenWidth : "" , demandPopin: true }) 
     	                	    	       	    	            								           	     ]
...
function buildObjectList(){
	...
	html5sql.process("SELECT * FROM MyObjectList where orderid = '"+CurrentOrderNo.replace(/^[0]+/g, "")+"' ;",
			 function(transaction, results, rowsArray){
				var n = 0;
				...
				while (n < rowsArray.length) {
					opTable.addItem (new sap.m.ColumnListItem("Obj:"+rowsArray[n].id,{
						...
						cells : 
							[
							new sap.m.Text({text: rowsArray[n].equipment.replace(/^[0]+/g, "")}),
							new sap.m.Text({text: rowsArray[n].equidescr}),
							new sap.m.CheckBox("check"+n,{select:function(evt){
...
function buildMultiReject() {
    ...
    html5sql.process("SELECT orderid , ordnOop,shortText,siteShcode FROM MyJobDets WHERE status = 'DEPL' or ( status = 'ACPT' and length(onsiteDate) <4 and length(parkDate) <4 )",
			 function (transaction, results, rowsArray) {
			     var n = 0;
			     var opTable = sap.ui.getCore().getElementById('MultiRejectTable');
