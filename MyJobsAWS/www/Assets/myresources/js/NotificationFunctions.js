	 ...
	 function(transaction, results, rowsArray){
		 ...
		 if (rowsArray.length>0){
			 ...
			 sap.ui.getCore().getElementById('notificationHEADER').addStatus( new sap.m.ObjectStatus({
			  ...
			  sap.ui.getCore().getElementById('notificationLongText').setText(rowsArray[0].shorttext+unescape(rowsArray[0].longtext))	
...
function deleteEntry(selItem){
	...
	html5sql.process("DELETE FROM "+tabName+" where id = "+id,
			 ...
			 function(error, statement){
				 opErrorMessage("Error: " + error.message + " when Deleting " + statement);
