...
function syncTransactional() {
  ...
  if (Date.parse(localStorage.getItem('LastLogoff')) < d) {
    ...
    logoutazure(function() {
      opMessage("SYNC:logoutazure from syncTransactional complete")
    });
  } else {
...
function syncUpload() {
  ...
  UploadSQLStatement += " union "
  /*changes for CREOL*/
  UploadSQLStatement += " select 'EquipmentStatus' as type,   '' as extra,id as id, recordupdated from MyEquipmentUpdates where state = 'NEW' "
  UploadSQLStatement += " union "
...
function syncReference() {
  if (!CheckSyncInterval('REFERENCE')) {
    return;
  }
  if (!checkConnection()) {
    return;
...
function buildDirs(fs) {
  ...
  entry.getDirectory("MyJobs", {
    create: true,
    exclusive: false
  },
    function(dir) {
      console.log("Created dir " + dir.name);
      entry.getDirectory("MyJobs/Global", {
        create: true,
        exclusive: false
      },
        function(dir) {
          console.log("Created dir " + dir.name);
          entry.getDirectory("MyJobs/Private", {
            create: true,
            exclusive: false
          },
            function(dir) {
              console.log("Created dir " + dir.name);
              entry.getDirectory("MyJobs/Private/Download", {
                create: true,
                exclusive: false
              },
                function(dir) {
                  console.log("Created dir " + dir.name);
                  entry.getDirectory("MyJobs/Private/Upload", {
                    create: true,
                    exclusive: false
                  },
                    function(dir) {
                      console.log("Created dir " + dir.name);
                      entry.getDirectory("MyJobs/Private/Photos", {
                        create: true,
                        exclusive: false
                      },
                        function(dir) {
                          console.log("Created dir " + dir.name);
                          entry.getDirectory("MyJobs/Global/Download", {
                            create: true,
                            exclusive: false
                          },
                            function(dir) {
                              console.log("Created dir " + dir.name);
                              entry.getDirectory("MyJobs/Global/Upload", {
                                create: true,
                                exclusive: false
                              },
                                function(dir) {
                                  console.log("Created dir Global-Upload" + dir.name);
                                  entry.getDirectory("MyJobs/Global/Forms", {
                                    create: true,
                                    exclusive: false
                                  },
                                    function(dir) {
                                      console.log("Created dir Global-Forms" + dir.name);
                                      entry.getDirectory("MyJobs/Private/Photos/ASC", {
                                        create: true,
                                        exclusive: false
                                      },
                                        function(dir) {
                                          console.log("Created dir Global-ASC" + dir.name);
                                          entry.getDirectory("MyJobs/Global/Icons", {
                                            create: true,
                                            exclusive: false
                                          },
                                            function(dir) {
...
function dropTables() {
  ...
  var sqlstatement = 'DROP TABLE IF EXISTS MyJobsParams;' +
...
var sqldeletetable = 'DELETE FROM MyJobsParams;' +
...
var sqlresettable = 'DELETE FROM MyJobsParams;' +
  'DELETE FROM DecommissionStatus;' +
  'DELETE FROM AssetSites;' +
  'DELETE FROM AssetSitesDetails;' +
  'DELETE FROM MyRefUsers;' +
      ...
      if (rowsArray.length > 0) {
        for (var i = 0; i < rowsArray.length; i++) {
          ...
          if (item.MODEL.startsWith('#')) {
            item.MODEL = item.MODEL.substr(1)
          }
          if (item.MODEL == model) {
            callback(true);
            break;
...
function populateModel(ModelData) {
  ...
  for (var cntx = 0; cntx < ModelData.Model.length; cntx++) {
    myarray.push({
      ...
      'data': [ModelData.Model[cntx].EQART, ModelData.Model[cntx].HERST, ModelData.Model[cntx].MODEL]
    })
  }
...
function populateEquipmentTypeCode(EquipmentTypeCodeData) {
  ...
  for (var cntx = 0; cntx < EquipmentTypeCodeData.EquipmentTypeCode.length; cntx++) {
    myarray.push({
      ...
      'data': [EquipmentTypeCodeData.EquipmentTypeCode[cntx].EARTX, EquipmentTypeCodeData.EquipmentTypeCode[cntx].ZATCODE,
...
function populateAssetTypeCodes(AssetTypeCodesData) {
  ...
  for (var cntx = 0; cntx < AssetTypeCodesData.AssetTypeCodes.length; cntx++) {
    myarray.push({
      ...
      'data': [AssetTypeCodesData.AssetTypeCodes[cntx].ZATCODE, AssetTypeCodesData.AssetTypeCodes[cntx].ZATDEF1,
...
function populateFunctionTypeCodes(FunctionTypeCodesData) {
  ...
  for (var cntx = 0; cntx < FunctionTypeCodesData.FunctionTypeCodes.length; cntx++) {
    myarray.push({
      ...
      'data': [FunctionTypeCodesData.FunctionTypeCodes[cntx].EARTX, FunctionTypeCodesData.FunctionTypeCodes[cntx].ZATCODE,
...
function populatePlantGroupCodes(PlantGroupCodesData) {
  ...
  for (var cntx = 0; cntx < PlantGroupCodesData.PlantGroupCodes.length; cntx++) {
    myarray.push({
      ...
      'data': [PlantGroupCodesData.PlantGroupCodes[cntx].ZPLGDEF1, PlantGroupCodesData.PlantGroupCodes[cntx].ZPLGDEF2,
...
function populateSystemCodes(SystemCodesData) {
  ...
  for (var cntx = 0; cntx < SystemCodesData.SystemCodes.length; cntx++) {
    myarray.push({
      ...
      'data': [SystemCodesData.SystemCodes[cntx].ZSYSCODE, SystemCodesData.SystemCodes[cntx].ZSYSDEF1,
...
function populateAssetCaptureCategory(AssetCaptureCategoryData) {
  ...
  for (var cntx = 0; cntx < AssetCaptureCategoryData.AssetCaptureCategory.length; cntx++) {
    myarray.push({
      ...
      'data': [AssetCaptureCategoryData.AssetCaptureCategory[cntx].ZASCAT, AssetCaptureCategoryData.AssetCaptureCategory[cntx].ZATCODE,
...
function populatePlantGroupAndProcessGroupCodes(PlantGroupAndProcessGroupCodesData) {
  ...
  for (var cntx = 0; cntx < PlantGroupAndProcessGroupCodesData.PlantGroupAndProcessGroupCodes.length; cntx++) {
    myarray.push({
      ...
      'data': [PlantGroupAndProcessGroupCodesData.PlantGroupAndProcessGroupCodes[cntx].ZPLGRP, PlantGroupAndProcessGroupCodesData.PlantGroupAndProcessGroupCodes[cntx].ZPRG,
