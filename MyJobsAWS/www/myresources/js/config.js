function loadConfigValues(){
    ...
    sap.ui.getCore().getElementById('configLastLogOff').setValue(localStorage.getItem('LastLogoff'));
				...
				content : [
							...
							new sap.m.Input("AdminDebugTokenValidity", { type: sap.m.InputType.Input }),
							 ...
							 new sap.m.Button({
				                    ...
				                    type: sap.m.ButtonType.Accept,
				                    tap: [function (oEvt) {
				                        logoutazure(function () { opMessage("SYNC:logoutazure from formAdminPW complete") });
				                    }]
				                }),
							 new sap.m.Button({
				                    text: "Reset DB",
...
function saveTheConfig(){
    ...
    SetConfigParam("LastLogoff", sap.ui.getCore().getElementById('configLastLogOff').getValue());
...
var formConfig = new sap.m.Dialog("dlgConfig",{
   ...
   // 						        postAzureData("ZGW_EQSTATUS_UPDATE", myjson)
							...
							new sap.m.Label({ text: "Last Log Off" }),
							new sap.m.Input("configLastLogOff", { type: sap.m.InputType.Input, enabled: true }),
