...
var formNewNotif = new sap.m.Dialog("dlgNewNotif",{
    ...
    buttons: [
					new sap.m.Button( {
					    ...
					    tap: [ function(oEvt) {	
					    	...
					    	var xpriority=sap.ui.getCore().byId("NewPriority").getSelectedItem().getKey()
	...
	beforeOpen:function(){
		...
		sap.ui.getCore().byId("NewNotifStart").setDateValue(new Date());
			 ...
			 function(transaction, results, rowsArray){
				...
				sap.ui.getCore().getElementById("NewPriority").addItem(
					...
					for (var n = 0; n < rowsArray.length; n++) {
						...
						sap.ui.getCore().getElementById("NewPriority").addItem(
								new sap.ui.core.Item({
									key: priority_type+"|"+item.priority,
